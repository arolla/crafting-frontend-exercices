...
export class BettingPage extends CustomHTMLElement {
  ...
  constructor() {
    ...
    window.addEventListener(
      UPDATE_BETS_SLIP,
      ((event: Event) => this.updateBetsSlip(event as CustomEvent)).bind(this),
    )
    window.addEventListener(
      UPDATE_STAKE,
      ((event: Event) => this.updateStake(event as CustomEvent)).bind(this),
    )
  }
  ...
  toggleSummaryRender() {
    const shouldDisplay = Boolean(
      this.stake && this.stake > 0 && this.betsSlip.length,
    )
  ...
  setSummaryAttribute(key: string, value: unknown) {
    const betsSummaryElement = this.shadowRoot?.querySelector(
      'arl-summary',
    ) as HTMLElement
